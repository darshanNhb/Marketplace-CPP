...
public:
    ...
    void display(bool detailed = false) const {
        ...
        if (isAuction) cout << " [AUCTION]";
        
        float rating = get_avg_rating();
        if (rating > 0) cout << " | Rating: " << setprecision(1) << rating << "/5";
        cout << endl;
...
public:
    ...
    void view_notifications() {
        cout << "\n--- NOTIFICATIONS ---" << endl;
        if (notifications.empty()) cout << "No new alerts." << endl;
    ...
    virtual void save_data() {
        ...
        fout << userId << "\n" << name << "\n" << password << "\n" << walletBalance << endl;
        for (auto& n : notifications) fout << "NOTIF:" << n << endl;
    }
    ...
    virtual bool load_data(const string& id) {
        ...
        string balStr;
        getline(fin, balStr); walletBalance = (balStr == "" ? 0.0f : stof(balStr));

        notifications.clear();
        string line;
        while (getline(fin, line)) {
            if (line.substr(0, 6) == "NOTIF:") notifications.push_back(line.substr(6));
        }
        return true;
    }
...
public:
    void load_inventory() {
        ...
        while (getline(fin, line)) {
            ...
            getline(ss, temp, ','); p.isAuction = (temp == "1");
            getline(ss, temp, ','); p.currentHighBid = stof(temp);
            getline(ss, p.highBidderId, ',');
            getline(ss, temp, ','); p.isClosed = (temp == "1");
            ...
            if (getline(ss, revs)) {
                ...
                string r;
                while (getline(ssr, r, ';')) {
                    if (!r.empty()) p.reviews.push_back(Review::deserialize(r));
                }
            }
            p.sellerId = userId;
    ...
    void save_inventory() {
        ...
        for (auto& p : myProducts) {
            ...
            for (auto& r : p.reviews) fout << r.serialize() << ";";
            fout << endl;
        }
    }
    ...
    void add_product() {
        ...
        cout << "Enter Category: "; cin.ignore(); getline(cin, p.category);
        cout << "Is this an Auction? (1=Yes, 0=No): "; int choice; cin >> choice;
        p.isAuction = (choice == 1);
        if (p.isAuction) p.currentHighBid = p.price;
    ...
    void view_sales_report() {
        ...
        if (!fin) { cout << "No sales." << endl; return; }
        cout << "\n--- SALES REPORT ---" << endl;
        string line;
        float totalIncome = 0;
        while (getline(fin, line)) {
            ...
            getline(ss, id, ','); getline(ss, bid, ','); getline(ss, sid, ',');
            if (sid == userId) {
                getline(ss, pid, ','); getline(ss, qty, ','); getline(ss, price, ','); 
                getline(ss, date, ','); getline(ss, type, ',');
    ...
    void menu(vector<Product>& marketplace) override {
        ...
        do {
            ...
            case 1:
                for (auto& p : myProducts) {
                    p.display();
                    if (p.quantity <= 5) cout << "   [WARNING: LOW STOCK!]" << endl;
                }
                break;
            case 2: add_product(); break;
            case 3: view_sales_report(); break;
            case 4: view_notifications(); save_data(); break;
            }
        } while (choice != 0);
    }
};
...
class Buyer : public User {
    struct CartItem { int pid; int qty; string sid; float price; string name; };
...
public:
    void view_marketplace(vector<Product>& marketplace) {
        ...
        for (auto& p : marketplace) {
            if (p.quantity > 0 || p.isAuction) p.display();
        }
    }
    ...
    void checkout(vector<Product>& marketplace) {
        ...
        float subtotal = 0;
        for (auto& item : cart) subtotal += item.price * item.qty;
        
        cout << "Total to Pay: $" << subtotal << endl;
        if (walletBalance < subtotal) { cout << "Insufficient funds!" << endl; return; }
        ...
        for (auto& item : cart) {
            ...
            t.save_to_file();
            // In a simple single file demo, cross-user updates are hard.
            // But we'll save the buyer data.
            for (auto& mp : marketplace) {
                if (mp.id == item.pid) mp.quantity -= item.qty;
            }
        }
        cart.clear();
        save_data();
    ...
    void menu(vector<Product>& marketplace) override {
        ...
        do {
            ...
            cin >> choice; cin.ignore();
            switch (choice) {
            case 1: view_marketplace(marketplace); break;
            case 2: {
                int id, q; cout << "Product ID: "; cin >> id;
                for (auto& mp : marketplace) {
                    if (mp.id == id) {
                        cout << "Qty: "; cin >> q;
                        if (q <= mp.quantity) cart.push_back({mp.id, q, mp.sellerId, mp.price, mp.name});
                        break;
                    }
                }
                break;
            }
            case 3: checkout(marketplace); break;
            case 4: place_bid(marketplace); break;
            case 5: { float amt; cout << "Amount: "; cin >> amt; add_balance(amt); save_data(); break; }
            case 6: view_notifications(); save_data(); break;
            }
        } while (choice != 0);
    }
};
...
public:
    Admin() { userId = "admin"; name = "Admin"; password = "as"; }
    void menu(vector<Product>& marketplace) override {
        int choice;
        do {
            cout << "\n--- ADMIN CONSOLE ---" << endl;
...
public:
    void init() {
        ...
        // Load marketplace from seller list
        ...
        while (fin >> sid) {
            ...
            while (getline(pin, line)) {
                ...
                getline(ss, p.name, ',');
                getline(ss, t, ','); p.price = stof(t);
                getline(ss, t, ','); p.quantity = stoi(t);
                getline(ss, p.category, ',');
                getline(ss, t, ','); p.isAuction = (t == "1");
                getline(ss, t, ','); p.currentHighBid = stof(t);
                getline(ss, p.highBidderId, ',');
                getline(ss, t, ','); p.isClosed = (t == "1");
                p.sellerId = sid;
    ...
    void run() {
        ...
        while (true) {
            ...
            if (t == 3) {
                string p; cout << "Pass: "; cin >> p;
                if (p == "as") admin.menu(marketplace);
                continue;
            }
            cout << "1. Login\n2. Signup\nChoice: ";
            int a; cin >> a;
            if (t == 1) {
                Seller s;
                if (a == 2) {
                    cout << "ID: "; string i; cin >> i;
                    cout << "Name: "; string n; cin.ignore(); getline(cin, n);
                    cout << "Pass: "; string ps; cin >> ps;
                    ofstream f("data/users/" + i + ".txt"); f << i << "\n" << n << "\n" << ps << "\n0\n";
                    ofstream fl("data/sellers_list.txt", ios::app); fl << i << "\n";
                    cout << "Signed up!";
                } else {
                    string i, p; cout << "ID: "; cin >> i; cout << "Pass: "; cin >> p;
                    if (s.login(i, p)) s.menu(marketplace);
                }
            } else if (t == 2) {
                Buyer b;
                if (a == 2) {
                    ...
                    cout << "Pass: "; string ps; cin >> ps;
                    ofstream f("data/users/" + i + ".txt"); f << i << "\n" << n << "\n" << ps << "\n0\n";
                    ofstream fl("data/buyers_list.txt", ios::app); fl << i << "\n";
                    cout << "Signed up!";
                } else {
                    string i, p; cout << "ID: "; cin >> i; cout << "Pass: "; cin >> p;
                    if (b.login(i, p)) b.menu(marketplace);
                }
            }
        }
    }
